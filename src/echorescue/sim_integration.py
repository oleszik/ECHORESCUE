...
def telemetry_progressed(samples: Sequence[Mapping[str, Any]]) -> bool:
    """Return whether a later position sample has a newer boot timestamp."""
    newest: int | float | None = None
    for sample in samples:
        timestamp = sample.get("time_boot_ms")
...
def diagnose(config: Mapping[str, Any], repository_root: Path = REPOSITORY_ROOT) -> dict[str, Any]:
    ...
    if sim_vehicle is None or not sim_vehicle.is_file():
        ...
        sitl_binary = Path(ardupilot_home, baseline["ardupilot"]["sitl_binary"])
        checks.append(check(
            "ardupilot_sitl.binary",
            Status.PASS if sitl_binary.is_file() else Status.FAIL,
            str(sitl_binary) if sitl_binary.is_file() else f"prebuild ArduCopter SITL before smoke testing: {sitl_binary}",
        ))
        help_code, help_output = _run_version([str(sim_vehicle), "--help"])
        no_rebuild = help_code == 0 and "--no-rebuild" in help_output
        checks.append(check(
            "ardupilot_sitl.no_rebuild",
            Status.PASS if no_rebuild else Status.FAIL,
            "pinned sim_vehicle.py supports --no-rebuild" if no_rebuild else "pinned sim_vehicle.py did not advertise --no-rebuild",
        ))
...
def _mavlink_samples(endpoint: str, minimum: int, timeout_s: float, stream_rate_hz: int) -> list[dict[str, Any]]:
    ...
    deadline = time.monotonic() + timeout_s
    try:
        heartbeat = connection.wait_heartbeat(timeout=min(timeout_s, 10.0))
        ...
        while time.monotonic() < deadline:
            ...
            if message is not None:
                ...
                if len(samples) >= minimum and telemetry_progressed(samples):
                    break
    finally:
        connection.close()
    return samples
...
def _retain_failure_logs(
    sources: Mapping[str, Path],
    output_path: Path | None,
    maximum_bytes: int,
) -> list[dict[str, Any]]:
    """Copy bounded log tails beside the requested report after a failed run."""
    if output_path is None:
        return []
    output_path.parent.mkdir(parents=True, exist_ok=True)
    retained: list[dict[str, Any]] = []
    for process_name, source in sorted(sources.items()):
        if not source.is_file():
            continue
        size = source.stat().st_size
        with source.open("rb") as input_log:
            if size > maximum_bytes:
                input_log.seek(size - maximum_bytes)
            contents = input_log.read(maximum_bytes)
        destination = output_path.with_name(f"{output_path.stem}.{process_name}.log")
...
def smoke(
    ...
    with tempfile.TemporaryDirectory(prefix="echorescue-v014-") as temp_dir:
        ...
        try:
            with log_sources["gazebo"].open("w", encoding="utf-8") as gazebo_log:
                ...
                with log_sources["ardupilot_sitl"].open("w", encoding="utf-8") as sitl_log:
                    ...
                    samples = _mavlink_samples(
                        endpoint["endpoint"],
                        int(config["health"]["minimum_telemetry_samples"]),
                        timeout_s,
                        int(config["health"]["position_stream_rate_hz"]),
                    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "diagnose":
        ...
        report = smoke(config, args.timeout, args.mode == "headless", args.output)
